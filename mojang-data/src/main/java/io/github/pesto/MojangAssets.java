...
final class MojangAssets {
    ...
    private void downloadJar(JsonObject versionInfo, @NotNull File destination) throws IOException {
        ...
        // Try to get server JAR first (contains full datapack contents), fallback to client JAR  
        JsonObject jarInfo = downloads.getAsJsonObject("server");
        String jarType = "server";
        
        if (jarInfo == null) {
            // Fallback to client JAR if server JAR is not available
            jarInfo = downloads.getAsJsonObject("client");
            jarType = "client";
            System.out.println("Warning: Server JAR not available, using client JAR (may have limited datapack content)");
        }
        
        String url = jarInfo.get("url").getAsString();
        System.out.println("Downloading " + jarType + " JAR for complete datapack contents...");
        ...
        try (InputStream input = connection.getInputStream()) {
            ...
            Loading.start(String.format("Downloading vanilla %s jar (%.2f MB)...", jarType, totalMB));
            long pos = 0;
            ...
            if (!success)
                throw new IOException("Failed to download " + jarType + " JAR");
        }
    }
